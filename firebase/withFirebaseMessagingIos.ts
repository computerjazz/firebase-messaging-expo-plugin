...
const withAppGroupPermissions: ConfigPlugin<OneSignalPluginProps> = (
  ...
  return withEntitlementsPlist(config, (newConfig) => {
    ...
    }.firebase-image-notification-service-extension`;
...
const withFirebaseMessagingPodfile: ConfigPlugin<OneSignalPluginProps> = (
  config
) => {
...
const withFirebaseMessagingNSE: ConfigPlugin<OneSignalPluginProps> = (
  config,
  props
) => {
  // support for monorepos where node_modules can be above the project directory.
  const pluginDir = require.resolve(
    "firebase-messaging-expo-plugin/package.json"
  );
  ...
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      await nseUpdater.updateNSEEntitlements(
        `group.${config.ios?.bundleIdentifier}.firebase-image-notification-service-extension`
      );
...
const withFirebaseMessagingXcodeProject: ConfigPlugin<OneSignalPluginProps> = (
  config,
  props
) => {
...
export const withFirebaseMessagingIos: ConfigPlugin<OneSignalPluginProps> = (
  config,
  props
) => {
